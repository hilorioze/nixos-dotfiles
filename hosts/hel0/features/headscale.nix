    ...
    headscale-setup = {
      ...
      script = ''
        ...
        ensure_user() {
          ...
          local user_json=$($headscale_cmd users list --output json --name $user_name 2>/dev/null)

          if [[ -n $user_json && $user_json != "[]" ]]; then
            ${lib.getExe pkgs.jq} --raw-output '.[0].id' <<<$user_json
          else
            $headscale_cmd users create $user_name --output json 2>/dev/null | ${lib.getExe pkgs.jq} --raw-output '.id'
          fi
        }
        ...
        set_node_tags_by_node_id() {
          ...
          if [[ -n $node_id ]]; then
            $headscale_cmd nodes tag --identifier $node_id --tags "$@"
          fi
        }
        ...
        rename_node_by_node_id() {
          ...
          if [[ -n $node_id ]]; then
            $headscale_cmd nodes rename --identifier $node_id $new_node_name
          fi
        }
